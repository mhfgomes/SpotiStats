...
import { getRecentlyPlayed, getTopArtists, getTopTracks, getTracksByIds } from "../lib/spotify";
import {
  buildTasteProfile,
  buildTopGenres,
...
interface RecentlyPlayedItemResult {
  trackSpotifyId: string;
  trackName: string;
  albumName: string;
...
export const getCurrentUserRecentlyPlayed = action({
  args: { limit: v.optional(v.number()) },
  handler: async (ctx, { limit = 50 }): Promise<RecentlyPlayedItemResult[]> => {
    const identity = await ctx.auth.getUserIdentity();
